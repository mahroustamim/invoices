			...
			<div class="nav nav-item  navbar-nav-right ml-auto">
				...
				<div class="dropdown nav-item main-header-notification">
					...
					<div class="dropdown-menu">
						<div class="menu-header-content bg-primary text-right">
							<div class="d-flex">
								<h6 class="dropdown-title mb-1 tx-15 text-white font-weight-semibold">ألاشعارات</h6>
								<a href="{{route('notification.markread')}}" class="badge badge-pill badge-warning mr-auto my-auto float-left">تعيين قراءة الكل</a>
							</div>
							<p class="dropdown-title-text subtext mb-0 text-white op-6 pb-0 tx-12 ">{{  auth()->user()->unreadNotifications->count(); }}</p>
						</div>
						...
						@foreach (auth()->user()->unreadNotifications  as $notification)
							<a class="d-flex p-3 border-bottom" href="{{ url('InvoicesDetails/' . $notification->data['id']) }}">
								...
								<div class="mr-3">
									<h5 class="notification-label mb-1">تم إضافة فاتورة جديدة</h5>
									<div class="notification-subtext">{{ $notification->data['user'] }} :  {{ $notification->created_at }}</div>
								</div>
